...
LotteryWallet/
  ...
  Features/Scan                 扫票：人工框选 → 矫正 → 配准 → 划格子 → 本机识别 → 复核
  Features/Stats                统计
...
图标已定稿为 18 号“无缺口圆角扇”：蓝、红、橙三张等尺寸圆角票据以左侧同一支点展开，角度依次为 `+12° / -6° / -24°`。票据不再使用左右缺口，整体更简洁；三层合成外轮廓已针对 1024×1024 画布重新校准居中。
...
3. 透视矫正 + 水平矫正 —— 按四个角拉成正矩形，再按文字基线角度转正
4. **整票识别一遍**，只取彩种 / 期号 / 开奖日期 / 玩法 / 倍数 / 合计金额
5. **配准** —— 用票面自己印的东西（虚线、机号行）定出号码区，摆成标准矩形
6. 号码按彩种分两条路读（见下）
7. 拼回整篇文本 → 解析 → 逐张复核，号码球可点开改
...
| | 号码取值 | 自带校验 | 走哪条 |
|---|---|---|---|
| 排列3 / 排列5 / 福彩3D / 七星彩 | 0–9，任意数字都合法 | **没有** | **格子路**：墨迹投影划格子，每个号码落进它那一格 |
| 双色球 / 大乐透 / 七乐彩 / 快乐8 | 两位数，升序、不重复、有值域 | **有** | 号码区整块喂一次，按行带拼回去 |

数字型彩种读错了没有任何办法发现，格子是它们唯一的护栏；两位数那几种
一注号码自己就是一道校验，再加格子是多余的。

### 三条硬约束

1. **每个号码都要说得出它来自票面的哪个像素。** 说不出来的标问号，
   **问号未补齐禁止导入票夹**。
2. **摆不上格子宁可整个矩阵作废。** 少认一注用户看得见，错位用户看不见。
3. **号码路和标签路彻底分开。**

一次一张票，全部识别在本机完成，照片不上传、不保存。
205 条单元测试锁住解析规则和版式判据，真票量出来的像素数写进了测试夹具。

> 详见 **[`Engineering/vision-system.md`](Engineering/vision-system.md)** ——
> 架构、实测常量、调试图怎么读、已经证伪不要再试的路。
> 推导过程在 [`Engineering/vision-rebuild.md`](Engineering/vision-rebuild.md)。
