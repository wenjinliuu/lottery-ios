...
/// 3. 本应用**不销售、不代购、不提供兑奖服务**，只记录用户已持有的实体票。
enum Disclaimer {
    /// 票夹卡片底部那一行。最短，因为每张卡片都要出现。
    static let card = "号码以实体票为准，中奖结果以官方兑奖为准"

    /// 扫描入口。用户还没拍照，先把预期摆正：这东西会出错，你得自己核。
    static let scanIntro = "识别结果可能有误，保存前请对照实体票逐注核对。本应用不销售、不代购、不提供兑奖服务。"
    ...
    /// 手动录入。
    static let entry = "请按实体票面如实录入。本应用仅记录你已持有的实体彩票，不销售、不代购、不提供兑奖服务。"
