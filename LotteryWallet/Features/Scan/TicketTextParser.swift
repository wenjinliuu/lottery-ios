...
enum TicketTextParser {
    ...
    static func normalize(_ raw: String) -> String {
        ...
        let replacements: [(String, String)] = [
            ...
            ("[（]", "("), ("[）]", ")"), ("[，,]", " "),
            ("[\t\u{000B}\u{000C}]+", " "), ("\n{2,}", "\n"),
            // Vision 偶尔会把注序号的 A/B/C 认成**西里尔字母**（А В С Е，
            // 长得一模一样但码位完全不同）。真实结果里出现过 `A. А.05 16 …`，
            // 那个西里尔 А 会让整行过不了"只有数字"这道闸，一注就这么丢了。
            ("[\u{0410}]", "A"), ("[\u{0412}]", "B"), ("[\u{0421}]", "C"),
            ("[\u{0415}]", "E"), ("[\u{041E}\u{043E}]", "O"),
            // 全角字母同理
            ("[ＡＢＣＤＥ]", "A")
        ]
    ...
    ///
    /// 这个 `?` 是 `RegisteredDigitReader` 划格子的产物：那一格在票面上的位置
    /// 说得出来，值没认出来。以前这种行只能整条丢掉 ——
    ...
    /// 行首的注序号。
    ///
    /// 末尾那个 `{1,2}` 是必须的：Vision 偶尔把同一个序号认出两遍
    /// （真实结果里有 `A. A.05 16 24 …`），只摘一层的话剩下的 `A.`
    /// 会让这一行过不了「只有数字」那道闸，整注被丢掉。
